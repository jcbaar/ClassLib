...
// Setup the disabled imagelist to use.
HIMAGELIST ClsBitmapMenu::SetDisabledImageList( HIMAGELIST hImages )
{
	// Get the old images.
	HIMAGELIST hOldImages = m_hDisabledImageList;

	// Set the new image-list.
	m_hDisabledImageList = hImages;
...
LRESULT ClsBitmapMenu::OnReflectedDrawItem( LPDRAWITEMSTRUCT pdis, BOOL& bNotifyParent )
{
	...
	if ( ! pData->m_bToplevel )
	{
		...
		// Clear icon area.
		cDC.FillRect( rcBitmap, ( HBRUSH )((UINT_PTR)nCol + 1 ));
		rcBitmap.Right()--;
		...
		if ( pData->m_iBitmap >= 0 && pData->m_pOwner->m_hImageList )
		{
			...
			// Disabled? We render using the disabled imagelist
			// if it is available...
			if( pdis->itemState & ODS_DISABLED && m_hDisabledImageList)
			{
				// Use the disabled image list.
				ClsDrawTools::RenderBitmap( pdis->hDC, pData->m_pOwner->m_hDisabledImageList, pData->m_iBitmap, rcBitmap, ClsDrawTools::CDSF_TRANSPARENT );
			}
			else
			{
				// Use the normal image list for both normal and
				// disabled images.
