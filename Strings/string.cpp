...
const ClsString& ClsString::operator+=( const ClsString& csSource )
{
	...
	// contents.
	int nLength = GetStringLength();
	AllocateBuffer( nLength + csSource.GetStringLength(), TRUE );

	// Append string.
	_tcscpy( &m_pszString[ nLength ], csSource.m_pszString );
	return *this;
}
...
const ClsString& ClsString::operator+=( LPCTSTR pszSource )
{
	...
	// contents.
	int nLength = GetStringLength();
	AllocateBuffer( nLength + ( int )_tcslen( pszSource ), TRUE );

	// Append string.
	_tcscpy( &m_pszString[ nLength ], pszSource );
	return *this;
}
