...
HWND ClsWindow::Detach()
{
	...
	if ( GetSafeHWND())
	{
		...
		if ( m_lpfnOldWndProc && m_lpfnOldWndProc != ( WNDPROC )ClsWindow::StaticWindowProc )
		{
			...
			::SetWindowLongPtr( m_hWnd, m_bIsDialog ? DWL_DIALOGPROC : GWL_WINDOWPROC, ( LONG_PTR )m_lpfnOldWndProc );
...
BOOL ClsWindow::Attach( HWND hWnd, BOOL bDestroy /* = FALSE */ )
{
	...
	if ( ::GetClassName( hWnd, szClassName, 10 ))
	{
		...
		m_lpfnOldWndProc = ( WNDPROC )::GetWindowLongPtr( hWnd, m_bIsDialog ? DWL_DIALOGPROC : GWL_WINDOWPROC );
		...
		if ( m_lpfnOldWndProc && m_lpfnOldWndProc != ( WNDPROC )ClsWindow::StaticWindowProc )
		{
			...
			// procedure.
			::SetWindowLongPtr( hWnd, m_bIsDialog ? DWL_DIALOGPROC : GWL_WINDOWPROC, ( LONG_PTR )ClsWindow::StaticWindowProc );
		}
		// Return success.
		return TRUE;
	}
...
BOOL ClsWindow::Create( DWORD dwExStyle, LPCTSTR lpszClassName, LPCTSTR lpszWindowName, DWORD dwStyle, int x, int y, int nWidth, int nHeight, HWND hwndParent, HMENU nIDorHMenu )
{
	...
	if ( PreCreateWindow( &cs ))
	{
		...
		if ( m_hWnd )
		{
			...
			// Get the window procedure.
			m_lpfnOldWndProc = ( WNDPROC )::GetWindowLongPtr( m_hWnd, m_bIsDialog ? DWL_DIALOGPROC : GWL_WINDOWPROC );
			...
			if ( m_lpfnOldWndProc && m_lpfnOldWndProc != ( WNDPROC )ClsWindow::StaticWindowProc )
			{
				...
				// Subclass the original window
				// procedure.
				if ( ::SetWindowLongPtr( m_hWnd, m_bIsDialog ? DWL_DIALOGPROC : GWL_WINDOWPROC, ( LONG_PTR )ClsWindow::StaticWindowProc ))
					// Return success.
					return TRUE;
			}
			else
	...
	if ( pWindow != NULL && pWindow->GetSafeHWND() )
	{
		...
		switch ( uMsg )
		{
			...
			case	WM_MEASUREITEM:
				...
				if ( wParam )
				{
					...
					if ( pChild )
					{
						...
						{
							// Set message result.
							pWindow->SetWindowLong64( DWL_MESSAGERESULT, ( LONG_PTR )lResult );
							lResult = TRUE;
						}
			...
			case	WM_NOTIFY:
				...
				lResult = pWindow->OnNotify( pNMHDR );
				break;
		}
