...
{
	{
		ClsCriticalSection crit;
		if ( ClsBalloonHelp::s_pHookMan == NULL )
		{
			ClsBalloonHelp::s_pHookMan = new HookMan();
		}
	}
	ClsBalloonHelp::s_pHookMan->AddClass( this );
...
ClsBalloonHelp::~ClsBalloonHelp()
{
   ClsBalloonHelp::s_pHookMan->RemoveClass( this );

   	{
		ClsCriticalSection crit;
		if ( ClsBalloonHelp::s_pHookMan != NULL )
		{
			if ( ClsBalloonHelp::s_pHookMan->m_lstClasses.empty() == true )
			{
				delete ClsBalloonHelp::s_pHookMan;
				ClsBalloonHelp::s_pHookMan = NULL;
			}
		}
	}
			...
			if ( dc.IsValid())
			{
				...
				if ( dcTmp1.CreateCompatibleDC( &dc ))
				{
					...
					if ( dcTmp2.CreateCompatibleDC( &dc ))
					{
						...
						ClsBitmap bmpIcon2, bmpIconScaled2;
						if ( bmpIcon2.CreateCompatibleBitmap( dc, bm.bmWidth, bm.bmHeight ) &&
						     bmpIconScaled2.CreateCompatibleBitmap( dc, cx, cy ))
						{
							// Code is braced so the ClsSelector object go out of
							// scope at the appropiate time...
							{
								// Select bitmaps.
								ClsSelector sel1( &dcTmp1, bmpIcon2 );
								ClsSelector sel2( &dcTmp2, bmpIconScaled2 );
							...
							SetIcon( bmpIconScaled2, m_crBackground );
						}
						// Delete dc2
						dcTmp2.DeleteDC();
					}
...
void ClsBalloonHelp::SetTimeout( unsigned int unTimeout )
{
	...
	if ( NULL != m_hWnd )
	{
		if ( m_unTimeout )
		{
			m_unTimerClose = SetTimer( ID_TIMER_CLOSE, ( UINT )m_unTimeout, NULL );
		}
		else
		{
...
void ClsBalloonHelp::SetKeyboardHook()
{
	...
	ClsBalloonHelp::s_pHookMan->SetKeyboardHook();
...
{
	if ( ! m_bKeyboardHook ) { return; }
	ClsBalloonHelp::s_pHookMan->RemoveKeyboardHook();
	m_bKeyboardHook = false;
}
...
void ClsBalloonHelp::SetMouseHook()
{
	...
	ClsBalloonHelp::s_pHookMan->SetMouseHook();
	m_bMouseHook = true;
}
...
{
	if ( ! m_bMouseHook ) { return; }
	ClsBalloonHelp::s_pHookMan->RemoveMouseHook();
	m_bMouseHook = false;
}
...
void ClsBalloonHelp::SetCallWndRetHook()
{
	...
	ClsBalloonHelp::s_pHookMan->SetCallWndRetHook();
...
{
	if ( ! m_bCallWndRetHook ) { return; }
	ClsBalloonHelp::s_pHookMan->RemoveCallWndRetHook();
	m_bCallWndRetHook = false;
}
...
ClsBalloonHelp::HookMan::HookMan()
{
...
ClsBalloonHelp::HookMan::~HookMan()
{
	while( m_nCountKeyboard > 0 )
	{ 
		RemoveKeyboardHook(); 
	}
	while( m_nCountMouse > 0 ) 
	{ 
		RemoveMouseHook(); 
	}
	while( m_nCountCallWnd > 0 )
	{ 
		RemoveCallWndRetHook(); 
	}
	m_hKeyboardHook = NULL;
...
void ClsBalloonHelp::HookMan::AddClass( ClsBalloonHelp* pClass )
{
	RemoveClass( pClass );
	m_cs.Lock();
...
void ClsBalloonHelp::HookMan::RemoveClass( ClsBalloonHelp* pClass )
{
	m_cs.Lock();
	std::list< ClsBalloonHelp * >::iterator itrEnd = m_lstClasses.end();
	for ( std::list< ClsBalloonHelp * >::iterator itr = m_lstClasses.begin(); itr != itrEnd; )
	{
		if ( *itr == pClass )
		{ 
			itr = m_lstClasses.erase( itr ); 
		}
		else
		{ 
			itr++; 
...
void ClsBalloonHelp::HookMan::SetKeyboardHook()
{
	m_cs.Lock();
	m_nCountKeyboard++;
...
void ClsBalloonHelp::HookMan::RemoveKeyboardHook()
{
	m_cs.Lock();
	m_nCountKeyboard--;
...
// Mouse hook
void ClsBalloonHelp::HookMan::SetMouseHook()
{
	m_cs.Lock();
	m_nCountMouse++;
...
void ClsBalloonHelp::HookMan::RemoveMouseHook()
{
	m_cs.Lock();
	m_nCountMouse--;
...
// Call Window Return hook
void ClsBalloonHelp::HookMan::SetCallWndRetHook()
{
	m_cs.Lock();
	m_nCountCallWnd++;
...
void ClsBalloonHelp::HookMan::RemoveCallWndRetHook()
{
	m_cs.Lock();
	m_nCountCallWnd--;
...
LRESULT CALLBACK ClsBalloonHelp::HookMan::KeyboardHookProcStatic( int code, WPARAM wParam, LPARAM lParam )
{
	ClsBalloonHelp::s_pHookMan->m_cs.Lock();
	try
	{
		std::list< ClsBalloonHelp * >::iterator itrEnd = ClsBalloonHelp::s_pHookMan->m_lstClasses.end();
		for ( std::list< ClsBalloonHelp * >::iterator itr = ClsBalloonHelp::s_pHookMan->m_lstClasses.begin(); itr != itrEnd; itr++ )
		{
			if ((( ClsBalloonHelp * )( *itr ))->KeyboardHookProc( code, wParam, lParam))
			{ 
				break; 
			} 
		}
		
		if ( NULL != ClsBalloonHelp::s_pHookMan->m_hKeyboardHook )
		{ 
			ClsBalloonHelp::s_pHookMan->m_cs.Unlock();
			return ::CallNextHookEx( ClsBalloonHelp::s_pHookMan->m_hKeyboardHook, code, wParam, lParam ); 
		}
	}
	catch( ... )
	{
...
LRESULT CALLBACK ClsBalloonHelp::HookMan::MouseHookProcStatic( int code, WPARAM wParam, LPARAM lParam )
{
	ClsBalloonHelp::s_pHookMan->m_cs.Lock();
	try
	{
		...
		for ( std::list< ClsBalloonHelp * >::iterator itr = ClsBalloonHelp::s_pHookMan->m_lstClasses.begin(); itr != itrEnd; itr++ )
		{
			if ((( ClsBalloonHelp * )( *itr ))->MouseHookProc( code, wParam, lParam ))
			{ 
				break; 
			} 
		}

		if ( NULL != ClsBalloonHelp::s_pHookMan->m_hMouseHook )
		{ 
			ClsBalloonHelp::s_pHookMan->m_cs.Unlock();
			return ::CallNextHookEx( ClsBalloonHelp::s_pHookMan->m_hMouseHook, code, wParam, lParam ); 
		}
	}
	catch( ... )
	{
...
LRESULT CALLBACK ClsBalloonHelp::HookMan::CallWndRetProcStatic( int code, WPARAM wParam, LPARAM lParam )
{
	ClsBalloonHelp::s_pHookMan->m_cs.Lock();
	try
	{
		...
		for ( std::list< ClsBalloonHelp * >::iterator itr = ClsBalloonHelp::s_pHookMan->m_lstClasses.begin(); itr != itrEnd; itr++ ) 
		{
			if ((( ClsBalloonHelp * )( *itr ))->CallWndRetProc( code, wParam, lParam)) 
			{ 
				break; 
			} 
		}

		if ( NULL != ClsBalloonHelp::s_pHookMan->m_hCallWndRetHook)
		{ 
			ClsBalloonHelp::s_pHookMan->m_cs.Unlock();
			return ::CallNextHookEx( ClsBalloonHelp::s_pHookMan->m_hCallWndRetHook, code, wParam, lParam); 
		}
	}
	catch( ... )
	{
...
ClsBalloonHelp::HookMan *ClsBalloonHelp::s_pHookMan = NULL;