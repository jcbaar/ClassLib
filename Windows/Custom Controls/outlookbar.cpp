...
void ClsOutlookBar::OnDrawOutlookButton( ClsDC *pDC, LPOUTLOOKDATA pData )
{
	...
	if ( pData->m_pszButtonTitle )
	{
		...
		// Render the label.
		pdc->DrawText( pData->m_pszButtonTitle, ( int )_tcslen( pData->m_pszButtonTitle ), pData->m_TitleBounds, dwFlags );
...
void ClsXPOutlookBar::OnDrawOutlookButton( ClsDC *pDC, LPOUTLOOKDATA pData )
{
	...
	if ( pData->m_pszButtonTitle )
	{
		...
		{
			pData->m_TitleBounds.Deflate( 2, 0 );
			dwFlags2 = DT_EXPANDTABS | DT_WORDBREAK | DT_VCENTER;
		}
		else 
			dwFlags2 = DT_EXPANDTABS | DT_WORDBREAK | DT_CENTER | DT_VCENTER;

		// Render the label.
		pdc->DrawText( pData->m_pszButtonTitle, ( int )_tcslen( pData->m_pszButtonTitle ), pData->m_TitleBounds, dwFlags2 );
