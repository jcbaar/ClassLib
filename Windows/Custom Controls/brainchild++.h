...
// brainchild++.h
//
// (C) Copyright 2000-2006 Jan van den Baard.
//     All Rights Reserved.
//
...
public:
	...
	inline DWORD GetLineLength( int nLine = -1 )
	{
		_ASSERT( GetSafeHWND() );
		return ( DWORD )SendMessage( BCM_GETLINELENGTH, ( WPARAM )nLine );
	}
	...
	inline int NumCharsInLine( int nLine = -1, BOOL bVirtual = FALSE )
	{
		_ASSERT( GetSafeHWND() );
		return ( int )SendMessage( BCM_NUMCHARSINLINE, ( WPARAM )nLine, ( LPARAM )bVirtual );
	}

	inline BOOL SetMarker( int nMarker, int nLine, BOOL bSingleton )
	{
		_ASSERT( GetSafeHWND() );
		return ( BOOL )SendMessage( BCM_SETMARKER, MAKEWPARAM( bSingleton, nMarker ), ( LPARAM )nLine );
	}

	inline BOOL ClearMarker( int nMarker, int nLine )
	{
		_ASSERT( GetSafeHWND() );
		return ( BOOL )SendMessage( BCM_CLEARMARKER, ( WPARAM )nMarker, ( LPARAM )nLine );
	}

	inline BOOL ToggleMarker( int nMarker, int nLine )
	{
		_ASSERT( GetSafeHWND() );
		return ( BOOL )SendMessage( BCM_TOGGLEMARKER, ( WPARAM )nMarker, ( LPARAM )nLine );
	}

	inline BOOL ClearMarkers( int nMarker )
	{
		_ASSERT( GetSafeHWND() );
		return ( BOOL )SendMessage( BCM_CLEARMARKERS, ( WPARAM )nMarker, 0 );
	}

	inline BOOL GotoMarker( int nMarker, int nDirection )
	{
		_ASSERT( GetSafeHWND() );
		return ( int )SendMessage( BCM_GOTOMARKER, ( WPARAM )nMarker, ( LPARAM )nDirection );
	}

	inline BOOL SetFindString( LPCTSTR pszString )
	{
		_ASSERT( GetSafeHWND() );
		return ( BOOL )SendMessage( BCM_SETFINDSTRING, 0, ( LPARAM )pszString );
	}

	inline BOOL SetReplaceString( LPCTSTR pszString )
	{
		_ASSERT( GetSafeHWND() );
		return ( BOOL )SendMessage( BCM_SETREPLACESTRING, 0, ( LPARAM )pszString );
	}

	void SetFindFlags( LONG dwFlags )
	{
		_ASSERT( GetSafeHWND() );
		SendMessage( BCM_SETFINDFLAGS, dwFlags, 0 );
	}

	BOOL SetParser( LPCTSTR pszFilename )
	{
		_ASSERT( GetSafeHWND() );
		return ( BOOL )SendMessage( BCM_SETPARSER, 0, ( LPARAM )pszFilename );
	}
...
protected:
	...
	virtual LRESULT OnMarkerStateChanging( LPNMMARKERSTATE pMarkerState, BOOL& bNotifyParent )	{ return 0; }
	virtual LRESULT OnMarkerStateChanged( LPNMMARKERSTATE pMarkerState, BOOL& bNotifyParent )	{ return 0; }
	virtual LRESULT OnMarginClick( LPNMMARGINCLICK pMarginClick, BOOL& bNotifyParent )		{ return 0; }
	virtual LRESULT OnFindChanged( LPNMFINDCHANGED pFindChanged, BOOL& bNotifyParent )		{ return 0; }
	...
	virtual LRESULT OnReflectedNotify( LPNMHDR pNMHDR, BOOL& bNotifyParent )
	{
		...
		switch ( pNMHDR->code )
		{
			...
			case	NMBC_MARKERSTATECHANGING:   return OnMarkerStateChanging(( LPNMMARKERSTATE )pNMHDR, bNotifyParent );
			case	NMBC_MARKERSTATECHANGED:    return OnMarkerStateChanged(( LPNMMARKERSTATE )pNMHDR, bNotifyParent );
			case	NMBC_MARGINCLICK:	    return OnMarginClick(( LPNMMARGINCLICK )pNMHDR, bNotifyParent );
			case	NMBC_FINDCHANGED:	    return OnFindChanged(( LPNMFINDCHANGED )pNMHDR, bNotifyParent );
		}
