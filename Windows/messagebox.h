...
protected:
	...
	// We render our background when the OS
	// is higher than 5.
	virtual LRESULT OnEraseBkgnd( ClsDC *pDC )
	{
		int osVersion = ClsGetApp()->GetMajorVersion();
		if ( osVersion < 6 )
		{
			return 0;
		}
		...
		{ 
			// Create the brushes.
			ClsBrush brush(  ::GetSysColor( COLOR_WINDOW )), 
			         brush2( ::GetSysColor( COLOR_3DFACE ));

			// Get the rectangles to render.
			ClsRect r1( m_HSlave1.GetWindowRect()), r2( m_Master.GetWindowRect());
			ScreenToClient( r1 );
			ScreenToClient( r2 );

			// Render the rectangles.
			pDC->FillRect( r2, brush2 );
			pDC->FillRect( r1, brush );
		}
		return 1;
	}

	// We render the background color ourselves.
	...
	virtual LRESULT OnInitDialog( PROPSHEETPAGE *pPropSheetPage )
	{
		...
		m_Master.Create( *this, LAYOUT_Horizontal, FALSE, LAYOUT_Master, TRUE, 
														  LAYOUT_Spacing, 0,
														  LAYOUT_LeftOffset, 0,
														  LAYOUT_TopOffset, 0, 
														  LAYOUT_RightOffset, 0,
														  LAYOUT_BottomOffset, 0,
														  LAYOUT_EqualMinWidth, TRUE,
														  TAG_END );
		m_HSlave1.Create( *this, LAYOUT_Horizontal, TRUE, LAYOUT_LeftOffset, osVersion >= 6 ? 20: 10,
														  LAYOUT_RightOffset, osVersion >= 6 ? 20: 10,
														  LAYOUT_TopOffset, osVersion >= 6 ? 20: 10,
														  LAYOUT_BottomOffset, osVersion >= 6 ? 20: 10,
														  LAYOUT_Spacing, 12,
														  TAG_END );
		m_HSlave2.Create( *this, LAYOUT_Horizontal, TRUE, LAYOUT_LeftOffset, 10, 
														  LAYOUT_TopOffset, 10, 
														  LAYOUT_RightOffset, 10, 
														  LAYOUT_BottomOffset, 8, 
														  LAYOUT_EqualMinWidth, TRUE, 
														  LAYOUT_EqualMinHeight, TRUE, 
														  TAG_END );
		...
		m_HSlave2.AddSpacingMember( osVersion >= 6 ? 150 : 75 );
		...
		// Add a second spacing member under XP or lower.
		// On Vista and higher the buttons are right aligned.
		if ( osVersion < 6 )
		{
		...
		if ( m_hIcon )
		{
			...
			if ( m_Icon.Create( this, ClsRect( 0, 0, 0, 0 ), ( UINT )-1, NULL, WS_CHILD | WS_VISIBLE | SS_ICON | WS_EX_TRANSPARENT ))
			{
				// Setup the icon.
		...
		// Force background color accoording to the
		// OS version.
		m_Body.BackgroundColor() = ::GetSysColor( ::ClsGetApp()->GetMajorVersion() < 6 ? COLOR_3DFACE : COLOR_WINDOW );
		...
		m_HSlave1.AddMember( &m_Body, NULL, ATTR_FixMinWidth, FALSE, ATTR_FixMinHeight, TRUE, TAG_END );

		// Add both horizontal layoutengines to the
		// vertical master layoutengine.
		m_Master.AddMember( &m_HSlave1, NULL, ATTR_FixMinWidth, FALSE, ATTR_FixMinHeight, TRUE, TAG_END );
		...
		m_Size.CY() += ::GetSystemMetrics( SM_CYFRAME ) + ::GetSystemMetrics( SM_CYCAPTION );
	...
	// Override for the window procedure.
	LRESULT WindowProc( UINT uMsg, WPARAM wParam, LPARAM lParam )
	{
		switch ( uMsg )
		{
			// We need to setup the correct background color
			// brush for the static control which displays the icon.
			case WM_CTLCOLORSTATIC:
			{
				return ( LRESULT )::GetSysColorBrush( ClsGetApp()->GetMajorVersion() >= 6 ? COLOR_WINDOW : COLOR_3DFACE );
			}
		}
