...
public:
	...
	// Get the position (32-bit)
	inline DWORD GetPos32( bool& bError )
	{
		_ASSERT_VALID( GetSafeHWND() ); // Must be valid.

		return ( DWORD )::SendMessage( m_hWnd, UDM_GETPOS32, 0, ( LPARAM )&bError );
	}

	// Get the position (32-bit)
	inline DWORD GetPos32()
	{
		_ASSERT_VALID( GetSafeHWND() ); // Must be valid.

		return ( DWORD )::SendMessage( m_hWnd, UDM_GETPOS32, 0, 0 );
	}
	...
	// Set the position (32bit).
	inline int SetPos32( int nPos )
	{
		_ASSERT_VALID( GetSafeHWND() ); // Must be valid.
